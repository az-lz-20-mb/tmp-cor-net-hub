module "hubnetworking_hub" {
  ...
  hub_virtual_networks = {
    for key, hub in var.hub_virtual_networks : key => {
      name                            = lookup(var.hub_name[hub.index], "name")
      ...
      route_table_name                = lookup(var.route_table_name[hub.index], "name")
      ...
      firewall = hub.firewall != null ? {
        ...
        name                  = lookup(var.firewall_name[hub.index], "name")
        sku_name              = hub.firewall.sku_name
        sku_tier              = hub.firewall.sku_tier
        firewall_policy = {
          name = lookup(var.firewall_policy_name[hub.index], "name")
          dns = {
      ...
      subnets = {
        for subnet_key, subnet in hub.subnets : subnet_key => {
          name             = lookup(var.subnet_name[subnet.index], "name")
